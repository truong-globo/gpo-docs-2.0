...
<details>
<summary>No email arrives</summary>

In order: the workflow is **Active**; the order actually contained app options; order data access was approved; and a test email arrives. If the test arrives and real orders do not, tell us.
</details>

<details>
<summary>The test email fails</summary>

Almost always the provider credentials. Re-enter them carefully — SMTP passwords and API keys are easy to mistype, and some providers require an application-specific password rather than your account password.
</details>

<details>
<summary>Emails go to spam</summary>

Send from your own authenticated domain using your own provider, and set **From email** to an address on that domain.
</details>

<details>
<summary>The email is missing the option details</summary>

The template's line item loop is not printing properties. Check it against the [Liquid variables reference](liquid-variables-reference.md), or use **Revert to default** and start again.
</details>

<details>
<summary>I broke the template</summary>

**Revert to default**. It restores the original, and cannot be undone.
</details>

<details>
<summary>Test sending is refused</summary>
