---
description: >-
  Price a product by the measurements the customer enters, using a formula on
  the Dimension option type.
icon: ruler-combined
---
...
120 × 180 = 21600, and 21600 × 0.01 = $216.00. If that price is not acceptable, your pricing is not linear. See [When pricing is not linear](dimension-formula.md#when-pricing-is-not-linear) below.
{% endstep %}
...
* **Dimension is not supported in Shopify POS.** For in-person orders, collect measurements with [Number](../option-types/input-types/number.md) fields and apply the price another way. See [POS limitations](../point-of-sale/limitations.md).
