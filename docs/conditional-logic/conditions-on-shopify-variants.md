---
description: >-
  Show or hide options based on the Shopify variant the customer selected — how
  to set it up, and what to check when it does not work.
icon: shuffle
---
...
* This requires the **advanced** level of conditional logic. On other plans you can select **Shopify variant** as a source, but the operator and value fields are locked. See [Compare plans](../plans-and-billing/compare-plans.md).
...
The field displays your current storefront language as a suffix. This matters if your store is translated. See [Translated variant names](conditions-on-shopify-variants.md#translated-variant-names) below.
{% endstep %}
...
<figure><img src="../.gitbook/assets/2026-09-04_10-21-52.png" alt="A condition using Shopify variant as its source with a typed variant name"><figcaption><p>The variant name is typed, not picked — and the field shows which language you are typing it in.</p></figcaption></figure>
...
{% hint style="info" %}
Use **contains** on a product with several variant dimensions. It avoids writing one condition per combination, and it keeps working when you add a new size.
...
See [Translate option content](../translations-and-languages/translate-option-content.md).
