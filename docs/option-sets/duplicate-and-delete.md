...
For example, if you duplicate an **Active** option set that applies to all products, the duplicate will also be **Active** and apply to all products. Both sets will have the same name, so customers may see the options twice until you update the duplicate.
...
This is usually what you want. If you need separate inventory, reconfigure the add-on product in the duplicate. See [Add-on pricing](../add-on-pricing/add-on-pricing.md).
...
<summary>Pattern: the same options, different for some customers</summary>
...
2. **Set the copy to Draft**, so it cannot reach customers while you edit.
3. **Change what differs** — prices, wording, extra or fewer options.
4. **Split the audiences so they cannot overlap.** On the **Customers** tab give the copy `Customer tags — is equal to — wholesale`, and give the original the opposite, `Customer tags — is not equal to — wholesale`. **Skip this and both sets render for the same customer.**
5. **Activate the copy.** Both are live, but only one applies to any given customer.
...
2. **Review its add-on products.** Products generated by the app are **not** deleted when you delete the option set. They remain in your Shopify catalog, so delete or archive them in Shopify admin if you no longer need them.
3. **Check your automations.** A workflow using dynamic order-tag mode can reference a specific option in a specific option set. Deleting that set will break the workflow. See [Order tags update](../automations/update-order-tags.md).
...
* Store-wide settings, colors, and translations.
