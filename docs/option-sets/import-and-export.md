...
By default, imported option sets are saved as **Draft**, so you can review them before they become visible to customers. Enable this option only when you are sure the imported sets are ready to go live.
{% endstep %}
...
2. **Test at least one imported set on a real product before uninstalling the old app.** Uninstalling the old app first can leave a gap that is immediately visible to customers.
...
* Colors, borders, typography, custom CSS, widget position and behavior — export from **Settings**. See [Import and export settings](../settings/import-export-settings.md).
