---
description: >-
  The 12 option types where the customer enters a value instead of choosing from
  a list.
icon: keyboard
---
...
Most input types share the following settings, which are documented in [Shared settings](../shared-settings/):
...
{% hint style="info" %}
Some types cannot have a price, such as a date field used for a delivery fee. Add the charge to a separate [Switch](switch.md) or [Checkbox](../selection-types/checkbox.md), and use [conditional logic](../../conditional-logic/conditional-logic.md) to display the field when it is needed.
{% endhint %}
