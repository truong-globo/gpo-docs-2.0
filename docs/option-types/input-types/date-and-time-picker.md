...
This setting translates the month and day names in the calendar. It is separate from your option labels, which are translated for each storefront language. See [Translate option content](../../translations-and-languages/translate-option-content.md).
...
To charge for a date-related choice, such as express delivery or a weekend slot, add the price to a separate [Switch](switch.md), [Radio button](../selection-types/radio-button.md), or [Checkbox](../selection-types/checkbox.md). Then display the date field with [conditional logic](../../conditional-logic/conditional-logic.md).
...
* Available on paid plans. **Limit date picker**, **Range** mode, and the custom calendar language may require a higher plan. See [Compare plans](../../plans-and-billing/compare-plans.md).
