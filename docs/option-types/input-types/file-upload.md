...
To charge for an upload, such as an artwork setup fee, add the price to a [Switch](switch.md) or [Checkbox](../selection-types/checkbox.md), then display the upload field with [conditional logic](../../conditional-logic/conditional-logic.md).

## Store-wide settings that affect it

<table><thead><tr><th width="290">Setting</th><th>Where</th><th>Effect</th></tr></thead><tbody><tr><td><strong>File preview</strong></td><td>Settings > Settings > General > Product page</td><td><strong>Show image if the uploaded file is a photo, otherwise show link</strong>, or <strong>Show link</strong> for everything.</td></tr><tr><td>Maximum file size</td><td>Your plan</td><td>Each plan allows a different size per file. See <a href="../../plans-and-billing/compare-plans.md">Compare plans</a>.</td></tr><tr><td><strong>Choose file</strong>, <strong>File uploading</strong>, <strong>Uploaded file:</strong> and similar wording</td><td>Settings > Translations</td><td>All editable per language.</td></tr></tbody></table>
