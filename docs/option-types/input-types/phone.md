...
To charge for a related service, such as a phone consultation, add the price to a [Switch](switch.md) and use [conditional logic](../../conditional-logic/conditional-logic.md) to display the phone field when the switch is on.
...
* Available on paid plans. International validation is separately plan-gated — see [Compare plans](../../plans-and-billing/compare-plans.md).
