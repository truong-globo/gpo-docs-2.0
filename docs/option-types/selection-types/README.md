...
Selection types display a list of **option values** you define, and the customer selects one or several. Each value can have its own price, and each one can be linked to a real product for inventory.

## Eleven option types

<table><thead><tr><th width="210">Type</th><th>Looks like</th><th>Reach for it when</th></tr></thead><tbody><tr><td><a href="select.md">Select</a></td><td>The browser's own dropdown</td><td>You want something plain and dependable</td></tr><tr><td><a href="dropdown.md">Dropdown</a></td><td>A styled dropdown</td><td>You want search, swatches, or the live preview</td></tr><tr><td><a href="color-dropdown.md">Color dropdown</a></td><td>A dropdown with a color per entry</td><td>Many colors, little vertical space</td></tr><tr><td><a href="image-dropdown.md">Image dropdown</a></td><td>A dropdown with a picture per entry</td><td>Many designs, little vertical space</td></tr><tr><td><a href="radio-button.md">Radio button</a></td><td>A vertical list, one choice</td><td>Values have long names or their own help text</td></tr><tr><td><a href="checkbox.md">Checkbox</a></td><td>A list, several choices</td><td>A menu of optional extras</td></tr><tr><td><a href="button.md">Button</a></td><td>A row of buttons</td><td>Short values such as sizes</td></tr><tr><td><a href="color-swatch.md">Color swatch</a></td><td>A grid of color chips</td><td>Color is the choice, and you want it visible</td></tr><tr><td><a href="image-swatch.md">Image swatch</a></td><td>A grid of pictures</td><td>Patterns, materials, or designs you photograph</td></tr><tr><td><a href="font-picker.md">Font picker</a></td><td>A list of fonts, drawn in those fonts</td><td>The customer chooses the lettering</td></tr><tr><td><a href="product-links.md">Product links</a></td><td>Links to other products</td><td>You want to send them elsewhere, not collect a choice</td></tr></tbody></table>

## Comparing the selection types

Use this table to compare what each type supports.
...
The following selection types also offer **Not allow deselect**, which prevents the customer from clearing their choice once selected: **Dropdown**, **Color dropdown**, **Image dropdown**, **Button**, **Color swatch**, and **Image swatch**. This setting is available when **Allow multiple** is off. See [Selection behaviour](../shared-settings/selection-behaviour.md).

## What they all share

* **Option values** — the list itself, with per-value prices, colors, images, and help text. See [Working with option values](../../option-sets/option-values.md).
...
On a selection type, the price is set on **each option value** rather than on the option. This is the main difference from the input types, because different choices usually cost different amounts.

Set the price in the values table's **Price** column. Each value can use a different mode: linked to an existing product, generated by the app, or a plain price. See [Where you can set add-ons](../../add-on-pricing/where-you-can-set-add-ons.md).

Values with no price are free.

## Swatch style and selection behavior are separate

**Swatch style** controls how an option looks, separately from how it behaves. A Checkbox can be displayed as color chips, and a Button can display images. The option type controls the _selection behavior_, and the style controls the _appearance_.
...
* For long lists, consider three approaches: add [search](../shared-settings/selection-behaviour.md#search-suggestion), use a [collapsible layout or slider](../shared-settings/collapsible-layouts-and-sliders.md), or split the list across two options using [conditional logic](../../conditional-logic/conditional-logic.md).
