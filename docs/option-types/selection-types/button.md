...
Supported as an **image layer**. Each value can draw an image onto the product photo when it is selected. The settings are image shape, background mode, size, position, rotation, clip area, and customer controls. See [Image layers](../../product-personalizer/layer-settings/image-layers.md).
