...
Supported as an **image layer**. Each selected value can draw an image onto the product photo. The settings are image shape, background mode, size, position, rotation, clip area, and customer controls. See [Image layers](../../product-personalizer/layer-settings/image-layers.md).
