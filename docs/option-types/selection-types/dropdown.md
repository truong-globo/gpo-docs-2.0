...
Settings are **Image shape**, **Background mode**, width, height, position, opacity, rotation, crop area, and customer controls. See [Image layers](../../product-personalizer/layer-settings/image-layers.md).
...
Prices are set on each option value, and each value supports all three modes. The option-level **Add-on quantity** controls how the charge scales, including **Mixed quantity** when **Allow multiple** is on, which gives every value its own quantity box.

See [Add-on pricing](../../add-on-pricing/add-on-pricing.md) and [Advanced add-on modes](../../add-on-pricing/advanced-add-on-modes.md).
...
**Allow multiple** on, **Max selections** `3`, each value priced, and **Add-on quantity** set to **Mixed quantity** so the customer can order two of one value.

**A design chooser with live preview**

**Swatch style** set to **Image**, an image on each value, and **Personalizer** on so the selected design is displayed on the product photo.
