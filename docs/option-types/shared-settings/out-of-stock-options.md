...
{% hint style="warning" %}
This setting applies only to option values linked to an add-on product, either an existing product or a product generated by the app. Values that use **Fixed amount** have no product and no inventory, so the setting has no effect. See [Add-on pricing](../../add-on-pricing/add-on-pricing.md).
{% endhint %}
