---
description: >-
  Allowed value and Text transform — control which characters customers can
  enter and how the capitalization of their input is normalized.
icon: keyboard
---

# Text input rules

Two settings, both available only on **Text** and **Textarea**. [Limits](limits.md) control _how much_ a customer can enter; these settings control _what_ they can enter and how the submitted text is formatted.
...
* The restriction is applied as the customer types. Blocked characters simply do not appear in the field.
...
{% hint style="warning" %}
Explain the restriction in [Help text](placeholder-and-help-text.md#help-text). A customer who types an apostrophe into a **Letters** field will see nothing happen, with no explanation. Something like `Letters only — no punctuation` makes the restriction clear.
{% endhint %}
...
* The transform is applied as the customer enters their text, so the value saved to the order is already normalized. You do not need to tidy it up before production.
...
<table><thead><tr><th width="300">Situation</th><th>Setting</th></tr></thead><tbody><tr><td>Names to engrave in one consistent style</td><td><strong>Capitalized</strong></td></tr><tr><td>Initials or a monogram</td><td><strong>UPPERCASE</strong></td></tr><tr><td>A stamped label in a lowercase typeface</td><td><strong>lowercase</strong></td></tr><tr><td>A gift message in the customer's own words</td><td><strong>Default</strong></td></tr><tr><td>Team names on a jersey, printed in caps</td><td><strong>UPPERCASE</strong></td></tr></tbody></table>
...
* The Personalizer live preview displays the transformed text, so what the customer sees on the product photo matches the value that will be produced. See [Text layers](../../product-personalizer/layer-settings/text-layers.md).
* Neither setting affects the option’s **Label**, **Name**, or **Help text** — they apply only to the customer’s input.
* Neither setting applies to other option types. For example, a dropdown’s values are predefined by you, so there is nothing for the customer to restrict or normalize.
