...
{% hint style="info" %}
**Conditional logic also works on static types.** You can display a warning only when a customer selects a particular option, display a size chart only for garments, or hide an entire [Section](section.md) until it is relevant. See [Conditional logic](../../conditional-logic/conditional-logic.md).
{% endhint %}
