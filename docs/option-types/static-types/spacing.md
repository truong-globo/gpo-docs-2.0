...
A gap that adds empty space between elements. Nothing is displayed, it only creates space.

## What customers see

Nothing is displayed. The options above and below it have more space between them.
...
* **Give an important field more room.** This makes it stand out rather than reading as another item in the list.
...
* In place of a [Section](section.md). If the fields belong together, group them in a section.
* **As a stack of Spacing options.** Use one Spacing option with a larger height instead.
* To fix general widget spacing. Use [custom CSS](../../storefront-display-and-design/custom-css.md) so the spacing is applied consistently across the widget.
...
`24`px between the personalization fields and the delivery fields, where a divider would be too strong a separation.

**Space that only appears when needed**

Conditional logic matching the group below it, so there is no unexplained gap when that group is hidden.
...
* Vertical only.
* The height is the same on desktop and mobile, so a large value that looks right on desktop can look like a mistake on a phone. Check the mobile preview.
