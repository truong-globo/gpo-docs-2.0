...
* The Personalizer feature is available on your plan. See [Compare plans](../plans-and-billing/compare-plans.md).
...
Add a [Text](../option-types/input-types/text.md) option and set on the **Basic** tab:
...
See [Image layers](layer-settings/image-layers.md), [Crop area](layer-settings/clip-area.md), and [Customer controls](layer-settings/customer-controls.md).
{% endstep %}
...
The upload option is now displayed only after the customer selects the photo printing charge. See [Conditional logic](../conditional-logic/conditional-logic.md).
{% endstep %}
...
Set up an [automation](../automations/automations.md) so these details reach your production team without opening Shopify admin.
{% endstep %}
{% endstepper %}
