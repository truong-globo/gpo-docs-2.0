...
As the customer makes selections, the app runs [conditional logic](../conditional-logic/conditional-logic.md), redraws [the live preview](../product-personalizer/personalizer.md), and updates the price preview.
...
Properties whose name starts with an underscore are internal to the app, and Shopify hides them. A custom template can still print them. See [Show options on orders](../storefront-display-and-design/show-options-on-orders/).
...
A storefront cannot change what a customer is charged. Only Shopify can. While the customer is shopping, the app displays a _preview_ of the total, and at checkout Shopify applies the actual prices of the add-ons that were selected.
