...
Add-on products are per store, so options that linked to a product on the old store have nothing to link to on the new one. Reconfigure each one. See [Add-on pricing](../add-on-pricing/add-on-pricing.md).
{% endstep %}
...
Workflows are configured per store. See [Automations](../automations/automations.md).
{% endstep %}
{% endstepper %}
...
* Importing and exporting settings may not be available on all plans. See [Compare plans](../plans-and-billing/compare-plans.md).
