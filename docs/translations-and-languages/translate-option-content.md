...
* Translating option content may not be available on all plans. If the language switcher is unavailable, see [Compare plans](../plans-and-billing/compare-plans.md).
...
Translation is done in the builder, for one option set at a time.
{% endstep %}

{% step %}
### Switch language with the control in the builder header

The control lists your storefront languages and marks your primary language.
{% endstep %}

{% step %}
### Edit the text fields

The fields now contain the translation for the selected language rather than the original. Enter the translated text.

The preview panel also displays the selected language, so you can check the result immediately.
{% endstep %}
...
{% step %}
### Switch back to your primary language and check the original text

This confirms you edited the translation and not the original.
{% endstep %}
...
A rule that works on your English storefront does not run on your French storefront until you switch language in the builder and enter the French variant name. The value field shows the current language as a suffix.
...
* Untranslated text falls back to your primary language rather than displaying blank, so a partially translated store still works.
* The builder preview displays the selected language, which is the quickest way to check a translation in context.
