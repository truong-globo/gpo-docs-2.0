---
description: >-
  The 11 option types where the customer chooses from a list you define — and how
...
Three of them also offer **Not allow deselect**, which stops the shopper clearing their choice once made: Dropdown, Color dropdown, Image dropdown, Button, Color swatch, and Image swatch — while **Allow multiple** is off. See [Selection behaviour](../shared-settings/selection-behaviour.md).
...
You set it in the values table's **Price** column, and each value can use a different mode — one linked to an existing product, another generated, a third with a plain price. See [Where you can set add-ons](../../add-on-pricing/where-you-can-set-add-ons.md).

Values with no price are free.

## Swatch style crosses the boundaries

**Swatch style** means a Checkbox can look like colour chips and a Button can look like pictures. The type controls the *selection behaviour*; the style controls the *appearance*.

So do not choose Color swatch purely because you want colours — choose it because you want a grid with slider support. If you want colours in a vertical list with help text per value, that is a Radio button with **Swatch style** set to **Color**. See [Swatch style and previews](../shared-settings/swatch-style-and-previews.md).
