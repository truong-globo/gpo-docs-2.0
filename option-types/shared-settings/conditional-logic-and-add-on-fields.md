...
The source of a condition can be another option in the same option set, **or the Shopify variant the customer selected**. This lets you display an option only for the Large size, for example.

#### Two things to know first

* The available operators depend on the type of the source option. A text source offers `contains` and character-count comparisons, a checkbox source offers selection-count comparisons, and a switch source offers only `is enabled` and `is disabled`.
...
**Section** also supports conditional logic. A rule on a section shows or hides everything inside it at once, so if you are applying the same rule to six options, put it on the section that contains them instead.

For full detail, see [Conditional logic](../../conditional-logic/), the [operators reference](../../conditional-logic/operators-reference.md), and [conditions based on Shopify variants](../../conditional-logic/conditions-on-shopify-variants.md).

## Price

Adds an extra charge to the option.

<table><thead><tr><th width="180">Tab</th><th>Basic Settings, under <strong>Add-on Settings</strong></th></tr></thead><tbody><tr><td>Default</td><td>No add-on</td></tr><tr><td>Available on</td><td>At <strong>option</strong> level: Text, Textarea, Number, Switch, Color picker. At <strong>option value</strong> level, in the values table's <strong>Price</strong> column: all nine selection types with values. <strong>Dimension</strong> has its own price and formula fields</td></tr></tbody></table>

{% hint style="info" %}
Where the price is set depends on the option type. An input option has one answer, so it has one price. A selection option has several choices that may cost different amounts, so the price is set on each value. See [Where you can set add-ons](../../add-on-pricing/where-you-can-set-add-ons.md).
{% endhint %}
...
For full detail, see [Add-on pricing](../../add-on-pricing/).
