...
Most Personalizer issues have one of four common causes: no background image, no content to display, a layer positioned for a different image, or a plan restriction. Work through the checklist first, then find the symptom that matches your issue.
...
That option type does not support the Personalizer. Twelve supported types include: Text, Textarea, Number, File upload, and eight selection types. See the [overview](./#the-twelve-supported-option-types).
...
They share a position. Give them different **Y-Axis** values, or use [conditional logic](../conditional-logic/) so only one is visible at a time.
...
The color has too little contrast with the background, or **the opacity** is too low. Check the color against your background image rather than the builder panel. Over customer photos, add a thin [stroke](layer-settings/effects.md#stroke).
...
Confirm that it is still selected, then save and reload. For custom fonts, check that the font is uploaded in **Settings > Settings > General > Custom fonts** and that custom fonts are included in your plan. See [Fonts](layer-settings/fonts.md).
...
**Background mode** is set to **Stretch**. Switch to **Cover**.
...
**Cover** crops the image to fill the shape. Either use **Contain**, which leaves empty space, or enable the upload option's image editor so the customer can crop the image. See [File upload](../option-types/input-types/file-upload.md).
...
When [contacting support](../help/contact-support.md), include the option set name, the option, its Personalizer settings, the product you are testing on, and a screenshot showing the result compared with what you expected.
